...
from maxtext.utils import maxtext_utils_nnx, max_utils
...
class TestLeafSqSum(unittest.TestCase):
  """Unit test for the leaf_sq_sum and l2norm_pytree implementation in max_utils.py."""

  def test_leaf_sq_sum_numeric_and_quantized(self):
    @dataclass
    class MockQArray:
      qvalue: Any

    qarray = MockQArray(qvalue=jnp.array([1, 2], dtype=jnp.int32))
    float_array = jnp.array([3.0, 4.0])
    none_leaf = None

    # test l2norm_pytree directly
    pytree = {
        "qarray": qarray,
    ...
    # Expected: sqrt( (1^2 + 2^2) + (3^2 + 4^2) ) = sqrt(5 + 25) = sqrt(30)
    norm = max_utils.l2norm_pytree(pytree)
    self.assertAlmostEqual(float(norm), jnp.sqrt(30.0), places=5)
