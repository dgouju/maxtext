...
def DEEPSEEKV4_HF_WEIGHTS_TO_SHAPE(config):
  ...
  mapping = {
      ...
      "model.hc_head.hc_fn": [4, hidden_size * 4],
  ...
  for layer_idx in range(num_hidden_layers):
    ...
    layer_mapping = {
        ...
        f"{layer_prefix}.self_attn.sinks": [64],
        ...
        # MHC
        f"{layer_prefix}.attn_hc.fn": [24, hidden_size * 4],
        f"{layer_prefix}.attn_hc.base": [24],
        f"{layer_prefix}.attn_hc.scale": [3],
        f"{layer_prefix}.ffn_hc.fn": [24, hidden_size * 4],
        f"{layer_prefix}.ffn_hc.base": [24],
        f"{layer_prefix}.ffn_hc.scale": [3],
    ...
    if layer_idx >= 2:
      ...
      if c_type == "csa":
        ...
        layer_mapping.update({
           ...
           f"{layer_prefix}.self_attn.compressor.indexer.q_b_proj.weight": [64 * 128, 1024],
           f"{layer_prefix}.self_attn.compressor.indexer.scorer.weights_proj.weight": [64, hidden_size],
