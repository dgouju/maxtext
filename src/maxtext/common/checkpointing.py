...
def _weight_mismatches(want, have, path=(), check_missing: bool = True):
  ...
  check against checkpoint metadata, whose leaves carry shapes but no values). Only shapes and
  structure can disagree: Orbax casts restored array dtypes.
  """
  if isinstance(want, dict):
    ...
    for k, v in want.items():
      ...
      if check_missing or nested is not None:
        out.extend(_weight_mismatches(v, nested, path + (k,), check_missing=check_missing))
    return out
  name = "/".join(str(p) for p in path)
  if have is None or (check_missing and isinstance(have, jax.ShapeDtypeStruct)):
    return (
