...
def flash_attention_kernel(
  ...
  def init():
    ...
    if sinks_ref is None and max_logit_estimate is None:
      ...
      l_scratch_ref[...] = exp(sink - jnp.full_like(l_scratch_ref, max_logit_estimate))  # pyrefly: ignore[bad-argument-type, unsupported-operation]
  ...
  def body(kv_compute_index, _, has_partial_mask=False):
    ...
    exp = jnp.exp2 if config.use_base2_exp else jnp.exp
    if max_logit_estimate is None:
      s_curr = exp(qk - jnp.tile(m_next, (1, bkv_repeats)))  # pyrefly: ignore[bad-argument-type]
    else:
    ...
    if max_logit_estimate is None:
      alpha_o = jnp.tile(alpha, (1, head_dim_v_repeats))  # pyrefly: ignore[bad-argument-type]
...
def _splash_attention_forward(
  ...
  if segment_ids is not None:
    ...
    q_segment_ids = jax.lax.broadcast_in_dim(segment_ids.q, (q_seq_len, NUM_LANES), (0,))  # pyrefly: ignore[bad-argument-type]
    kv_segment_ids = jax.lax.broadcast_in_dim(segment_ids.kv, (NUM_SUBLANES, kv_seq_len), (1,))  # pyrefly: ignore[bad-argument-type]
  else:
    in_specs += [None, None]
  ...
  cost_estimate = config.fwd_cost_estimate or _fwd_cost_estimate(*vmem_inputs, out_shapes, fwd_mask_sparsity)  # pyrefly: ignore[bad-argument-count, bad-argument-type]
  ...
  def manual_sharding_spec(self, sharding: jax.sharding.NamedSharding):
    ...
    def mask_info_spec(mask_info):
      ...
      return MaskInfo(  # pytype: disable=wrong-arg-types
          ...
          kv_sequence=(jax.sharding.PartitionSpec() if mask_info.kv_sequence is not None else None),  # pyrefly: ignore[bad-argument-type]
      )
