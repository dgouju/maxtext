...
def get_nnx_var_named_sharding_with_scan_axis(v: nnx.Variable, mesh) -> nnx.Variable:
  ...
  if not hasattr(val, "shape"):
    ...
    if jax.tree_util.tree_leaves(val):
      ...
      if hasattr(first_leaf, "shape"):
        ...
        leaf_sharding = getattr(leaf_var, "sharding", NamedSharding(mesh, P()))
        return v.replace(sharding=leaf_sharding)
      replicated = NamedSharding(mesh, P())
      return v.replace(sharding=replicated)
    return v
  ...
  if not out_sharding:
    ...
    return v.replace(sharding=out_sharding)
    ...
    if pspec is not None and mesh is not None and hasattr(val, "shape"):
      pspec = adjust_pspec_for_indivisible_shapes(pspec, val.shape, mesh)
...
def _analyze_sharding(params, mesh, valid_target_mesh_axes):
  ...
  for path, p_leaf in all_params_leaves:  # Iterate over each parameter leaf
    ...
    if not is_sharded_on_all_target_axis:
      leaf_nbytes = getattr(p_leaf, "nbytes", getattr(p_leaf, "size", 0) * getattr(p_leaf, "itemsize", 1))
      unsharded_params_total_size += leaf_nbytes  # Add to total unsharded parameter size
      ...
      problematic_tensors_details.append(
          {
              ...
              "size": leaf_nbytes,  # tensor size in bytes
