  ...
  with nn_partitioning.axis_rules(config.logical_axis_rules):
    ...
    if config.pure_nnx:
      ...
      if getattr(getattr(config, "lora", None), "enable_lora", False) and getattr(config.lora, "lora_restore_path", None):
        # Standalone LoRA adapter restore path (Step 4 of LoRA E2E flow):
        # Restores standalone LoRA adapter weights onto the base model state after initialization.
        target_model_state = (
            state["model"]
            if (isinstance(state, (nnx.State, dict)) and "model" in state)
            else getattr(state, "model", state)
        ...
        _, state_mesh_annotations, state_mesh_shardings = get_abstract_state_nnx(config, mesh, init_state_fn, is_training)
    else:
      if restored:
        if isinstance(
            checkpoint_manager,
